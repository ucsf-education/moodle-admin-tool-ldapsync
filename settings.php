...
if ($hassiteconfig) {
    ...
    if ($ADMIN->fulltree) {
        ...
        if (!function_exists('ldap_connect')) {
            ...
            $settings->add(new admin_setting_heading('tool_ldapsync/pluginname', '',
                                                     new lang_string('ldapsync_description', 'tool_ldapsync')));

            // Target authtype setting, e.g. manual, shibboleth
            $enabledauth = array('manual'=>'manual');
            $moreauths = $DB->get_field('config', 'value', array('name' => 'auth'));
            if (!empty($moreauths)) {
            ...
            $settings->add(new admin_setting_configselect('tool_ldapsync/authtype',
                                                          new lang_string('authtype', 'tool_ldapsync'),
                                                          new lang_string('authtype_description', 'tool_ldapsync'),
                                                          \tool_ldapsync\importer::MOODLE_AUTH_ADAPTER, $enabledauth));

            // LDAP server settings.
            $settings->add(new admin_setting_heading('tool_ldapsync/ldapserversettings',
                                                     new lang_string('auth_ldap_server_settings', 'auth_ldap'), ''));

            // Host.
            $settings->add(new admin_setting_configtext('tool_ldapsync/host_url',
            ...
            $versions[2] = '2';
            $versions[3] = '3';
            $settings->add(new admin_setting_configselect('tool_ldapsync/ldap_version',
            ...
            $settings->add(new admin_setting_configselect('tool_ldapsync/start_tls',
                                                          new lang_string('start_tls_key', 'auth_ldap'),
                                                          new lang_string('start_tls', 'auth_ldap'), 0 , $yesno));


            // Encoding.
            $settings->add(new admin_setting_configtext('tool_ldapsync/ldapencoding',
            ...
            $settings->add(new admin_setting_configtext('tool_ldapsync/pagesize',
                                                        get_string('pagesize_key', 'auth_ldap'),
                                                        get_string('pagesize', 'auth_ldap'), '250', PARAM_INT));

            // Bind settings.
            $settings->add(new admin_setting_heading('tool_ldapsync/ldapbindsettings',
                                                     new lang_string('auth_ldap_bind_settings', 'auth_ldap'), ''));

            // User ID.
            $settings->add(new admin_setting_configtext('tool_ldapsync/bind_dn',
                                                        get_string('auth_ldap_bind_dn_key', 'auth_ldap'),
                                                        get_string('auth_ldap_bind_dn', 'auth_ldap'), '', PARAM_RAW_TRIMMED));

            // Password.
            $settings->add(new admin_setting_configpasswordunmask('tool_ldapsync/bind_pw',
                                                                  get_string('auth_ldap_bind_pw_key', 'auth_ldap'),
                                                                  get_string('auth_ldap_bind_pw', 'auth_ldap'), ''));

            // User Lookup settings.
            $settings->add(new admin_setting_heading('tool_ldapsync/ldapuserlookup',
                                                     new lang_string('auth_ldap_user_settings', 'auth_ldap'), ''));

            // User Type.
            $settings->add(new admin_setting_configselect('tool_ldapsync/user_type',
            ...
            // Contexts.
            $settings->add(new auth_ldap_admin_setting_special_contexts_configtext('tool_ldapsync/contexts',
                                                                                   get_string('auth_ldap_contexts_key', 'auth_ldap'),
                                                                                   get_string('auth_ldap_contexts', 'auth_ldap'), '', PARAM_RAW_TRIMMED));

            // Search subcontexts.
            $settings->add(new admin_setting_configselect('tool_ldapsync/search_sub',
            ...
            $settings->add(new admin_setting_configselect('tool_ldapsync/opt_deref',
                                                          new lang_string('opt_deref_key', 'tool_ldapsync'),
                                                          new lang_string('opt_deref', 'tool_ldapsync'), LDAP_DEREF_NEVER , $optderef));

            // User attribute.
            $settings->add(new auth_ldap_admin_setting_special_lowercase_configtext('tool_ldapsync/user_attribute',
                                                                                    get_string('auth_ldap_user_attribute_key', 'auth_ldap'),
                                                                                    get_string('auth_ldap_user_attribute', 'auth_ldap'), '', PARAM_RAW));

            // Suspended attribute.
            $settings->add(new auth_ldap_admin_setting_special_lowercase_configtext('tool_ldapsync/suspended_attribute',
                                                                                    get_string('auth_ldap_suspended_attribute_key', 'auth_ldap'),
                                                                                    get_string('auth_ldap_suspended_attribute', 'auth_ldap'), '', PARAM_RAW));

            // Member attribute.
            $settings->add(new auth_ldap_admin_setting_special_lowercase_configtext('tool_ldapsync/memberattribute',
            ...
            $settings->add(new admin_setting_configselect('tool_ldapsync/removeuser',
            ...
            $settings->add(new admin_setting_configselect('tool_ldapsync/sync_suspended',
        ...
        display_auth_lock_options($settings, 'tool_ldapsync', $authplugin->userfields,
